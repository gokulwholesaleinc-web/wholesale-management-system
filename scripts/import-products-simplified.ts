...
async function main() {
  try {
    console.log('Starting simplified product import...');
    ...
    console.log('Sample record:', JSON.stringify(records[4]));
    ...
    for (const record of records) {
      try {
        ...
        if (!categoryName && record.Department) {
          categoryName = record.Department;
        }
      ...
      if ((imported + skipped) % 50 === 0) {
        console.log(`Progress: ${imported} imported, ${skipped} skipped, ${skippedTobacco} tobacco skipped`);
      }
    }
