...
- **Redundant Page Cleanup**: Removed orphaned AdminStaffManagement.tsx page that had no active routes and duplicated functionality available in /admin/users and /admin/staff. This eliminates code duplication and maintenance overhead while preserving all user management functionality through proper admin routes.
- **Privacy Policy Modal Consolidation**: Removed duplicate PrivacyPolicyModal component (/components/PrivacyPolicyModal.tsx) that was redundant with the comprehensive auth version (/components/auth/PrivacyPolicyModal.tsx). System now uses single privacy policy modal for both post-login updates and new account creation, preventing user confusion with multiple privacy dialogs.
...
- **Advanced Address Normalization Implementation**: Implemented comprehensive address helper function that normalizes ANY delivery/customer address into one line. Supports multiple address formats (JSON objects, formatted strings, various field naming conventions), intelligent delivery vs pickup address handling, robust field extraction with case-insensitive key matching, and dynamic text wrapping in PDF invoice headers. Customer information section now properly displays wrapped addresses with dynamic card height adjustment, business name display when different from customer name, and professional formatting that handles long addresses gracefully across delivery and pickup orders.
- **PDF Invoice Customer Information Layout Enhancement**: Restructured customer information display in invoice PDF generation with proper separation of business name (bold), customer name (normal), and wrapped delivery address (up to 3 lines). Removed duplicate customer display logic from header card and consolidated all customer information into main body section with professional formatting and proper text wrapping for long addresses.
