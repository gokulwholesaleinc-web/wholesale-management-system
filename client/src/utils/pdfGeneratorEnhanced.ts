...
/** ------------ Enhanced PDF Generator with Pre-computed Credit Balance ------------ */
export const generateOrderPDF = async (
  order: Order, 
  customerName?: string,
  previousBalance?: number // Pre-computed credit balance parameter
) => {
  const doc = new jsPDF();
  ...
  // --------- Use Pre-computed Previous Balance ---------
  const creditBalance = previousBalance ?? 0;

  // Amount due = previous balance + this order (server snapshot)
  const amountDue = creditBalance + serverThisOrderTotal;
  ...
  doc.text(fmt$(creditBalance), 195, sy, { align: "right" });
  sy += 4;
