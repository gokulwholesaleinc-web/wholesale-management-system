      ...
      try {
        ...
        if (trustedDevice) {
          ...
          if (trustedDevice.createdAt > thirtyDaysAgo) {
            ...
            const mainToken = createAuthToken(user.id); // Main JWT for API calls
            ...
            return res.json({
              ...
              token: posToken,
              mainToken: mainToken, // Main JWT token for API calls
              user: {
                id: user.id,
                username: user.username,
                firstName: user.firstName,
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    // Generate POS session token AND main JWT token
    const timestamp = Date.now();
    const posToken = `pos-${user.id}-${timestamp}`;
    const mainToken = createAuthToken(user.id); // Main JWT for API calls
    ...
    res.json({
      ...
      token: posToken, // POS-specific token for compatibility
      mainToken: mainToken, // Main JWT token for API calls
