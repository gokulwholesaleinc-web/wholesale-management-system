...
import { z } from 'zod';
import { DB, canTransition, recalc, pushHistory, seedOrders } from '../services/orderCalculations';
...
// POST /api/new-orders/:id/payments
router.post('/:id/payments', requireAuth, (req, res) => {
  ...
  const list = DB.payments.get(o.id) || [];
  list.push({
    id: crypto.randomUUID(),
    provider: body.provider,
    external_id: body.external_id,
    amount: body.amount,
    type: body.amount >= 0 ? 'charge' : 'refund',
    status: 'succeeded',
    created_at: new Date().toISOString()
  });
  DB.payments.set(o.id, list);

  DB.orders.set(o.id, recalc(o));
  res.json({ data: DB.orders.get(o.id) });
});

// PATCH /api/new-orders/:id/item/:itemId  (edit qty or unit_price)
...
// POST /api/new-orders/:id/recalc  (force totals consistency)
